...
Game.prototype.updateStatus = function () {
	...
	if (that.board.score() == -that.score) {
		that.status = 1;
		that.markWin();
		document.getElementById.innerHTML = `<span>You have won! </span>`;
	...
	if (that.board.score() == that.score) {
		that.status = 2;
		that.markWin();
		document.getElementById.innerHTML = `<span>You have lost! </span>`;
	}

	// Tie
	if (that.board.isFull()) {
		that.status = 3;
		document.getElementById.innerHTML = `<span>Tie </span>`;
