$(function () {
		...
		$('.fullpage').fullpage(
			{
				afterRender: function () {
					...
					var next = $('<a href="#" class="fp-next"><i class="fas fa-gifts" aria-hidden="true"></i></a>');
				...
				navigation: true,
				normalScrollElements: '.modal-open .modals',
	...
	for (let i = 0; i < nodeList.length; i++) {
		...
		let textNode = document.querySelectorAll("#text")[i];
		let text = new CircleType(textNode);
		text.radius(150);
	...
	$(".door-card").on("dblclick", function () {
		var attr = $(this).attr("data-id");
		var card = $(this);
		var knockAudio = $("#knock")[0];
		var openAudio = $("#open-door")[0];
		knockAudio.play();

		setTimeout(function () {
			openAudio.play();
			card.find(".left-door *").addClass("left-door-open");
			card.find(".right-door *").addClass("right-door-open");
		}, 2000);

		setTimeout(function () {

			var modal = document.getElementById(attr);
		...
		events(document.getElementById(attr));
		card.find(".left-door *").removeClass("left-door-open");
		card.find(".right-door *").removeClass("right-door-open");
